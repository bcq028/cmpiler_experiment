...
ir::Value *ir::Executor::get_des_operand(Operand op)
{
    ...
    auto iter = cur_ctx->mem.find(op.name);
    if (iter != cur_ctx->mem.end())
    { // find the operand in current context
        retval = &iter->second;
    }
    else
    {
        ...
        { // find the operand in global variable
            retval = &iter->second;
        }
        else
        { // both not, then create a new one
#if (DEBUG_EXEC_DETAIL)
...
int getarray(int a[])
{
    ...
    for (int i = 0; i < n; i++)
        fscanf(ir::reopen_input_file, "%d", &a[i]);
    return n;
}
...
void putarray(int n, int a[])
{
    ...
    for (int i = 0; i < n; i++)
        fprintf(ir::reopen_output_file, " %d", a[i]);
