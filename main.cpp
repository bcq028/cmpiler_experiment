...
int main(int argc, char **argv)
{
    ...
    std::streambuf *original_cout_buffer = std::cout.rdbuf(); // 保存原始的 std::cout 缓冲区
    std::cout.rdbuf(file.rdbuf());                            // 将 std::cout 的缓冲区重定向到文件
#endif
    ...
    std::cout.rdbuf(original_cout_buffer); // 恢复原始的 std::cout 缓冲区
    file.close();
#endif // 关闭文件流
    return 0;
}